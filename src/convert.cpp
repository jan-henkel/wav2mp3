...
void pad_pcm(wav_t& wav) {
  ...
  unique_ptr<uint8_t[]> input(move(wav.data));
  wav.data.reset(new uint8_t[num_blocks*block_sz_out]);
  memory_layout::pad_le(wav.data.get(), input.get(), block_sz_in, block_sz_out, num_blocks);
...
void decode_ulaw(wav_t& wav) {
  ...
  for(int i=0;i<samples_total;++i) {
    ...
    mantissa<<=(exponent+1);
    *(short*)out = sgn*((mantissa+(33<<exponent)-33)<<(block_out*8-14));
  }
  wav.block_sz=block_out;
  wav.format_code=WAVE_FORMAT_PCM;
}
...
void check_support(const wav_t& wav) {
  ...
  else if(wav.format_code==WAVE_FORMAT_ALAW || wav.format_code==WAVE_FORMAT_MULAW) {
    if(wav.block_sz>1)
...
void decode(wav_t& wav) {
  check_support(wav);
  if(wav.format_code==WAVE_FORMAT_PCM && wav.block_sz!=sizeof(short) && wav.block_sz!=sizeof(int)) {
  ...
  if(wav.format_code==WAVE_FORMAT_PCM || wav.format_code==WAVE_FORMAT_IEEE_FLOAT)
    host_align_data(wav);
}
...
void set_lgf(lame_global_flags* lgf,const wav_t& wav) {
  ...
  lame_set_mode(lgf, wav.num_channels==1?MPEG_mode::MONO : MPEG_mode::JOINT_STEREO);
...
namespace convert {
  ...
  void convert(string filename_in, string filename_out) {
    ...
    unique_ptr<lame_global_flags,decltype(&lame_close)> lgf(lame_init(),&lame_close);
    set_lgf(lgf.get(),wav);
    ...
    unique_ptr<FILE,decltype(closer)> file_out_2(fopen(filename_out.c_str(), "rb+"),closer);
